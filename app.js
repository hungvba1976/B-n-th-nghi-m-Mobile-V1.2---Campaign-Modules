...
const APP_VERSION = "Mobile V1.2.2 Final";
...
const STORAGE_PREFIX = "luyenthi:v12:";
...
const CAMPAIGN_CACHE_NAME = "luyenthi-v12-campaign-modules";
...
function saveSession(session){ session.savedAt=Date.now(); localStorage.setItem(`${STORAGE_PREFIX}${session.storageKey}`,JSON.stringify(session)); }
function deleteSession(storageKey){ localStorage.removeItem(`${STORAGE_PREFIX}${storageKey}`); }
function loadSession(storageKey){
  const raw=localStorage.getItem(`${STORAGE_PREFIX}${storageKey}`); if(!raw)return null;
  try{
...
function mergeCampaignRegistry(catalog){
  ...
  catalog.filter(x=>campaignStatus(x)!=="EXPIRED").forEach(x=>
    byId.set(x.moduleId,{moduleId:x.moduleId,name:x.name,startDate:x.startDate,endDate:x.endDate})
  );
...
function scheduleCampaignMaintenance(){
  ...
  next.setHours(24,0,5,0); // 00:00:05 ngày kế tiếp
