...
const APP_VERSION = "Mobile V1.2.4.1 Session Hotfix";
...
function renderCampaignPracticeMode(campaign){
  ...
  $("#startCampaignPractice").onclick=()=>{
    const key=campaignStorageKey(campaign.moduleId,"practice");
    const existing=loadSession(key);
    if(existing&&!confirm("Đang có phiên luyện tập đã lưu của cuộc thi này. Tạo phiên mới sẽ thay thế phiên cũ. Tiếp tục?"))return;
    const s=newCampaignPractice(campaign,mode);
    deleteSession(key);
    saveSession(s);
    activateSession(s);
  };
...
function startCampaignExam(campaign){
  const key=campaignStorageKey(campaign.moduleId,"exam");
  const existing=loadSession(key);
  if(existing){
    if(!confirm("Đang có bài thi thử đã lưu của cuộc thi này. Tạo đề mới sẽ thay thế bài cũ. Tiếp tục?"))return;
  }else{
    if(!confirm(`Tạo đề ${campaign.examQuestionCount} câu / ${campaign.examMinutes} phút và bắt đầu ngay?`))return;
  }
  const s=newCampaignExam(campaign);
  deleteSession(key);
  saveSession(s);
  activateSession(s);
}
