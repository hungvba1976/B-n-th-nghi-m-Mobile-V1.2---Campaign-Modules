...
const CAMPAIGN_CACHE_NAME = "luyenthi-campaign-modules";
...
function campaignStatus(entry,today=localDateYmd()){
  if(today<entry.startDate)return "UPCOMING";
  if(today>entry.endDate)return "EXPIRED";
  return "ACTIVE";
}
function campaignDaysRemaining(entry,today=new Date()){
  const end=new Date(`${entry.endDate}T23:59:59`);
  return Math.max(0,Math.ceil((end-today)/(24*60*60*1000)));
}
function campaignPrefix(id){return `luyenthi:campaign:${id}:`;}
...
async function purgeCampaignCache(id){
  if(!("caches" in window))return;
  try{
    const cache=await caches.open(CAMPAIGN_CACHE_NAME);
    const reqs=await cache.keys();
    await Promise.all(reqs.filter(r=>new URL(r.url).pathname.includes(`/campaigns/${id}/`)).map(r=>cache.delete(r)));
  }catch{}
}
async function cleanupExpiredCampaigns(){
  const expired=CAMPAIGN_CATALOG.filter(x=>campaignStatus(x)==="EXPIRED");
  for(const entry of expired){
    const prefix=`luyenthi:${campaignStorageKey(entry.moduleId,"")}`;
    const toDelete=[];
    for(let i=0;i<localStorage.length;i++){
      const key=localStorage.key(i);
    ...
    localStorage.removeItem(`${campaignPrefix(entry.moduleId)}lastResult`);
    await purgeCampaignCache(entry.moduleId);
    campaignCache.delete(entry.moduleId);
  }
}
async function loadCampaign(entry){
  ...
  const qPayload=await fetchJson(module.questionsUrl);
...
function campaignQuestionData(campaign){
  const bankId=`campaign_${campaign.moduleId}`;
  return {
    bankVersion:`campaign:${campaign.moduleId}:${campaign.version}`,
    manifest:[{id:bankId,name:campaign.name,group:"Campaign"}],
    banks:{[bankId]:{meta:{id:bankId,name:campaign.name,group:"Campaign",newCount:0},questions:campaign.questions}}
  };
}
...
function campaignCards(){
  ...
  return `<section class="panel"><div class="row between"><div><div class="eyebrow">CUỘC THI PHONG TRÀO</div><div class="section-title mt8">Đang mở ôn luyện</div></div><span class="badge new">${items.length} cuộc thi</span></div>
    <div class="choice-list mt12">${items.map(x=>`<button class="btn3d campaign-card" data-campaign="${x.moduleId}">
      ...
      <div class="campaign-copy"><div class="strong">${esc(x.name)}</div><div class="small mt8">Còn ${campaignDaysRemaining(x)} ngày · đến ${formatDate(x.endDate)}</div>${x.demo?`<div class="badge new mt8">BẢN MÔ PHỎNG</div>`:""}</div>
    </button>`).join("")}</div>
  </section>`;
}
...
function renderCampaignHome(campaign){
  ...
  <section class="notice info"><b>Thời gian ôn luyện:</b> ${formatDate(campaign.startDate)} – ${formatDate(campaign.endDate)}<div class="mt8">${esc(campaign.description||"")}</div><div class="mt8"><b>${campaign.questionCount}</b> câu trong module.</div></section>
  <section class="grid2">
  ...
  <div class="notice mt12">Sau ngày ${formatDate(campaign.endDate)}, module sẽ tự ẩn và dữ liệu riêng của cuộc thi sẽ được xóa khỏi thiết bị.</div>`;
...
function renderCampaignPracticeMode(campaign){
  ...
  <button class="btn3d primary mt12" style="width:100%" id="startCampaignPractice">Bắt đầu luyện</button></section>`;
  ...
  $("#startCampaignPractice").onclick=()=>{const s=newCampaignPractice(campaign,mode);deleteSession(s.storageKey);saveSession(s);activateSession(s);};
}
function startCampaignExam(campaign){
  if(!confirm(`Tạo đề ${campaign.examQuestionCount} câu / ${campaign.examMinutes} phút và bắt đầu ngay?`))return;
  const s=newCampaignExam(campaign);deleteSession(s.storageKey);saveSession(s);activateSession(s);
}
function renderHome(){
